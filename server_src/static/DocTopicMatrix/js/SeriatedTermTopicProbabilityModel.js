...
var SeriatedTermTopicProbabilityModel = Backbone.Model.extend({
	defaults : {
		...
		"docIndex" : null,           /** @type{string[]}   A list of term texts. **/
		"docTerm" : null,
...
SeriatedTermTopicProbabilityModel.prototype.load = function () {
	var sumColumns = function(){
		...
		for ( var i = 0; i < this.get("topicIndex").length; i++ ){
			var sum = 0.0;
			for( var j = 0; j < this.get("docIndex").length; j++){
				sum += matrix[j][i];
			}
			colSums.push(sum);
		}
	...
	var successHandler = function(model, response, options) {
		console.log("Loaded SeriatedTermTopicProbabilityMode 0703", model, response, options);
	...
	var errorHandler = function(model, response, options) {
		console.log("Cannot load SeriatedTermTopicProbabilityModel 0703", model, response, options);
	}.bind(this);
	this.fetch({
		add : true,
...
SeriatedTermTopicProbabilityModel.prototype.printHTML = function( target ) {
	var termIndex = this.get("docIndex").slice()
	termIndex.sort();
